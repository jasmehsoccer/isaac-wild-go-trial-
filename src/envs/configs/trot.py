"""Config for Go2 speed tracking trotting environment."""
...
def get_config():
    ...
    config.goal_lb = torch.tensor([0.3, 0.], dtype=torch.float)  # Lin_x, Lin_y, Rot_z
    ...
    config.action_lb = np.array([-10., -10., -10., -20., -20., -20.]) * 0.2
    config.action_ub = np.array([10., 10., 10., 20., 20., 20.]) * 0.2
    ...
    # config.base_position_kp = np.array([0., 0., 50])
    # config.base_position_kd = np.array([10., 10., 10.])
    # config.base_orientation_kp = np.array([50., 50., 0.])
    ...
    config.swing_foot_height = 0.2
    config.swing_foot_landing_clearance = 0.5
