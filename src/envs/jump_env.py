...
from src.robots import go2, go2_robot
...
class JumpEnv:
    ...
    def __init__(self,
        ...
        self.ha_teacher = HATeacher(teacher_cfg=teacher_config)
        ...
        from src.utils.sim_utils import add_ground, add_terrain
    ...
    def step(self, action: torch.Tensor):
        ...
        com_action = action
        # print(f"com_action: {com_action}")
        foot_action = None
        # print(f"action: {action}")
        # print(f"gait_action: {gait_action}")
        # print(f"com_action: {com_action}")
        # print(f"foot_action: {foot_action}")
        ...
        # print(f"env_dt: {self._config.env_dt}")
        ...
        for step in range(max(int(self._config.env_dt / self._robot.control_timestep), 1)):
            ...
            self._robot.energy = self.ha_teacher.update(
                np.asarray(self._torque_optimizer.tracking_error.cpu()).squeeze())
            ha_action, dwell_flag = self.ha_teacher.get_action()

            if ha_action is not None:
            ...
            terminal_stance_ddq, action_mode = self.coordinator.get_terminal_action(hp_action=hp_action,
                                                                                    ...
                                                                                    plant_state=np.asarray(
                                                                                        self._torque_optimizer.tracking_error.cpu()).squeeze(),
            ...
            logs.append(
                dict(timestamp=self._robot.time_since_reset,
                     ...
                     energy=to_torch(self._robot.energy, device=self._device),
    ...
    def episode_length_buf(self, new_length: torch.Tensor):
        ...
        self._gait_generator._current_phase += 2 * torch.pi * (
                                                                      new_length / self.max_episode_length * self._config.get(
                                                                  'max_jumps', 1)
                                                                      + 1)[:, None]
    ...
    def add_lane(self):
        ...
        lane_half_width = 0.4
        # 创建 box 几何体（左，中，右）
        box_geom_left = self._gym.create_box(self._sim, track_length * 2, track_width * 2, track_height * 2)
        box_geom_middle = self._gym.create_box(self._sim, track_length * 2, track_width * 2, track_height * 2)
        box_geom_right = self._gym.create_box(self._sim, track_length * 2, track_width * 2, track_height * 2)

        env_lower = gymapi.Vec3(0., 0., 0.)
        env_upper = gymapi.Vec3(0., 0., 0.)

        env_handle = self._gym.create_env(self._sim, env_lower, env_upper, 1)

        # 创建材质属性（红色和蓝色）
        red_color = gymapi.Vec3(1, 0, 0)
        blue_color = gymapi.Vec3(0, 0, 1)

        # 设置左、中、右轨道的变换
        box_origin_x = 0
        left_transform = gymapi.Transform()
        left_transform.p = gymapi.Vec3(box_origin_x, lane_half_width, track_height)
        middle_transform = gymapi.Transform()
        middle_transform.p = gymapi.Vec3(box_origin_x, 0, track_height)
        right_transform = gymapi.Transform()
        right_transform.p = gymapi.Vec3(box_origin_x, -lane_half_width, track_height)

        # 创建左轨道
        left_actor = self._gym.create_actor(env_handle, box_geom_left, left_transform, "track_left", 0, 0)

        # self._gym.set_actor_color(None, left_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, red_color)

        # 创建中轨道
        # middle_actor = self._gym.create_actor(env_handle, box_geom_middle, middle_transform, "track_middle", 0, 0)
        # self._gym.set_actor_color(None, middle_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, blue_color)

        # 创建右轨道
        # right_actor = self._gym.create_actor(env_handle, box_geom_right, right_transform, "track_right", 0, 0)
        # self._gym.set_actor_color(None, right_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, red_color)
    ...
    def load_plane_asset(self):
        # 创建一个环境
        env_lower = gymapi.Vec3(0., 0., 0.)
        env_upper = gymapi.Vec3(0., 0., 0.)
        env = self._gym.create_env(self._sim, env_lower, env_upper, 100)

        # 加载一个 URDF 资产（或其他格式，如 OBJ、USD）
        asset_root = "meshes"  # 资产文件所在的目录路径
        asset_file = "plane.urdf"  # 资产文件的名称

        # 创建资产描述符并加载资产
        asset_options = gymapi.AssetOptions()
        asset_options.fix_base_link = True  # 是否固定基础链接，设置为 True 则物体不会移动
        ...
        asset_pose.p = gymapi.Vec3(0.0, 0.0, 0.001)  # 设置资产的位置
        asset_pose.r = gymapi.Quat(0.0, 0.0, 0.0, 1.0)  # 设置资产的旋转

        # 创建 Actor 并将资产放置到环境中
        # actor_handle = self._gym.create_actor(env, asset, asset_pose, "snow_road", 0, 0)

        # 加载雪地纹理
        # texture_path = "./meshes/snow_road.jpg"  # 替换为你的雪地纹理文件路径
        # snow_texture_handle = self._gym.create_texture_from_file(self._sim, texture_path)
        #
        # # 创建地面几何体（一个大盒子来代表雪地）
        # # ground_dims = gymapi.Vec3(10.0, 10.0, 0.001)  # 地面的尺寸
        ...
        #
        # # 将雪地纹理应用到地面 actor
        # self._gym.set_rigid_body_texture(env, actor_handle, 0, gymapi.MESH_VISUAL, snow_texture_handle)
        #
