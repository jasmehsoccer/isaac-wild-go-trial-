...
def compute_safe_acc(
    ...
    #                      [-0., -0., 0., 0, -0., 194]])
    #
    # patch_kd = np.array([[31., 0., 0., -0., 0., 0.],
    #                      [0., 13., -0., 0., -0, 0.],
    #                      [0., -0., 28., 0., -0., 0.],
    #                      [0., 0., -0., 26, 0., 0.],
    #                      [-0., 0., 0., -0., 26., -0.],
    ...
    # teacher_action = np.squeeze(patch_kp @ (self._plant_state[:6] - self._patch_center[:6]) * -1
    #                             + patch_kd @ (self._plant_state[6:] - self._patch_center[6:]) * -1)
...
class QPTorqueOptimizer:
    ...
    def get_safe_action(self, foot_contact_state: torch.Tensor,
        ...
        print(f"desired_ang_acc: {self.desired_angular_acceleration}")
        print(f"desired_lin_acc: {self.desired_linear_acceleration}")
        # time.sleep(1)

        if safe_acc is None:
            res = self.compute_joint_command(
    ...
    def get_action(self, foot_contact_state: torch.Tensor,
        ...
        if residual_acc is None:
            print(f"self._desired_angular_acceleration: {self._desired_angular_acceleration}")
            print(f"self._desired_linear_acceleration: {self._desired_linear_acceleration}")
            res = self.compute_joint_command(
                foot_contact_state=foot_contact_state,
                desired_base_orientation_rpy=self._desired_base_orientation_rpy,
                desired_base_position=self._desired_base_position,
            ...
            return self.compute_joint_command(
                ...
                desired_linear_velocity=self._desired_linear_velocity,
                desired_foot_velocity=torch.zeros(12),
                desired_angular_acceleration=self._desired_angular_acceleration + residual_acc[:, 3:6],
                desired_linear_acceleration=self._desired_linear_acceleration + residual_acc[:, :3],
                desired_foot_acceleration=torch.zeros(12))
    ...
    def tracking_error(self, device='cuda:0'):
        ...
        ang_pos_error = -1 * compute_orientation_error(self.desired_base_orientation_rpy,
                                                       base_quat,
                                                       device=device)
