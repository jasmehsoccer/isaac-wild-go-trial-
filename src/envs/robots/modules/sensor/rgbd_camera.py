...
class RGBDCamera:
    ...
    def _camera_gpu_warmup(self):
        ...
        depth_image = self._gym.get_camera_image(self._sim, self._env_handle, self._camera_handle, gymapi.IMAGE_DEPTH)
    ...
    def get_current_frame(self, img_save=True, img_show=True):
        ...
        depth_colored = np.zeros((*depth_img.shape, 3), dtype=np.uint8)
        depth_colored[depth_mask] = cv2.applyColorMap(depth_normalized[depth_mask], cv2.COLORMAP_JET).reshape(-1, 3)
        ...
        # Save image
        if img_save:
            rgb_folder = f"{self._img_save_folder}/rgb"  # RGB
            depth_folder = f"{self._img_save_folder}/depth"  # Depth
            depth_color_folder = f"{self._img_save_folder}/depth_color"  # Depth Color
            os.makedirs(rgb_folder, exist_ok=True)
            os.makedirs(depth_folder, exist_ok=True)
            os.makedirs(depth_color_folder, exist_ok=True)
            cv2.imwrite(f'{rgb_folder}/rgb_{self._video_cnt}.png', rgb_img)
            cv2.imwrite(f'{depth_folder}/depth_{self._video_cnt}.png', depth_normalized)
            cv2.imwrite(f'{depth_color_folder}/depth_color_{self._video_cnt}.png', depth_colored)
            self._video_cnt += 1
        ...
        self._frame_cnt += 1
        return rgb_img, depth_normalized
    ...
    def get_bev_map(self,
        ...
        if len(raw_pcd_wd) == 0:
            print("Warning: Empty point cloud detected, returning default BEV map")
            # Return default map
            ...
            if as_occupancy:
                # Return free space (all 1s) for occupancy map
                return np.ones((x_bins, y_bins), dtype=np.uint8)
            else:
                # Return zeros for height map
                return np.zeros((x_bins, y_bins), dtype=np.float32)
        ...
        # Check if ROI points are empty
        if len(roi_points) == 0:
            print("Warning: No points in ROI, returning default BEV map")
            x_bins = int((x_max - x_min) / grid_size)
            y_bins = int((y_max - y_min) / grid_size)
        ...
        if as_occupancy:
            ...
            # Check if we have any valid points for interpolation
            valid_mask = ~np.isnan(interp_grid)
            if np.any(valid_mask) and np.sum(valid_mask) > 3:  # Need at least 3 points for interpolation
                interp_grid[np.isnan(interp_grid)] = griddata((x[valid_mask], y[valid_mask]),
                                                              interp_grid[valid_mask],
                                                              (x[np.isnan(interp_grid)], y[np.isnan(interp_grid)]))
            else:
                # If not enough points, fill with default values
                interp_grid[np.isnan(interp_grid)] = z_low + (z_high - z_low) / 2
    ...
    def get_depth_origin_world_frame(self, min_threshold=-10001):
        """
        Get the origin of the depth image in world frame
        :param min_threshold: Minimum depth threshold
        :return: origin_in_world_frame, flag
        """
        self._gym.render_all_camera_sensors(self._sim)
        self._gym.start_access_image_tensors(self._sim)
        ...
        # Inverse of View Matrix with error handling
        try:
            view_mat_inv = np.linalg.inv(np.matrix(view_mat))
        except np.linalg.LinAlgError:
            # If matrix is singular, return default values
            print("Warning: Singular view matrix detected, using default origin")
    ...
    def get_pcd_data(self, in_world_frame=True, visualize=False,
        ...
        try:
            ...
            # If matrix is singular, return empty point cloud
            print("Warning: Singular view matrix detected in get_pcd_data, returning empty point cloud")
    ...
    def resolution(self) -> List:
        return [self._img_width, self._img_height]import time
...
nano src/envs/robots/modules/sensor/rgbd_camera.py
